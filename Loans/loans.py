...
# --- Load data
file_path = "D:/MFS/Works/Articles/Loans/Loans by sectors.xlsx"
df = pd.read_excel(file_path, sheet_name=0)

# Prepare structure
data = df.iloc[2:].copy()
data.columns = ["Sector","Currency","Residency"] + list(df.columns[3:])
for c in ["Sector","Currency","Residency"]:
...
for col in ["Sector","Currency"]:
    res_data = res_data[~res_data[col].str.contains("Total", case=False, na=False)]

# Aggregate flows (residents only)
flows = res_data.groupby(["Residency","Currency","Sector"], as_index=False)["Value"].sum()
...
currency_colors  = {"AMD":"goldenrod", "FX":"#EF4444"}  # AMD blue, FX purple
...
x_res, x_cur, x_sec = 0.00, 0.49, 0.99
...
for n, yy in zip(sec_items, sec_y):
    add_node(" ", sector_colors.get(n, "#cbd5e1"), x_sec, yy)  # placeholder label
...
# Currency -> Sector
ct = flows.groupby(["Currency","Sector"])["Value"].sum().reset_index()
for _, row in ct.iterrows():
    s = node_index[row["Currency"]]
    t = sector_node_indices[row["Sector"]]
    v = float(row["Value"])
    sources.append(s); targets.append(t); values.append(v)
    link_colors.append(sector_colors.get(row["Sector"], "#cbd5e1"))
    pct = v/grand_total*100 if grand_total>0 else 0
    link_hovers.append(f"{row['Currency']} → {row['Sector']}<br>{v:,.0f} mln AMD ({pct:.1f}%)")

# # Residency -> Sector
# rs = flows.groupby(["Residency","Sector"])["Value"].sum().reset_index()
...
#     link_colors.append(sector_colors.get(row["Sector"], "#cbd5e1"))
#     pct = v/grand_total*100 if grand_total>0 else 0
#     link_hovers.append(f"{row['Residency']} → {row['Sector']}<br>{v:,.0f} mln AMD ({pct:.1f}%)")

# # Sector -> Currency
# sc = flows.groupby(["Sector","Currency"])["Value"].sum().reset_index()
# for _, row in sc.iterrows():
#     s = sector_node_indices[row["Sector"]]
#     t = node_index[row["Currency"]]
#     v = float(row["Value"])
#     sources.append(s); targets.append(t); values.append(v)
#     link_colors.append(currency_colors.get(row["Currency"], "#9CA3AF"))
#     pct = v/grand_total*100 if grand_total>0 else 0
#     link_hovers.append(f"{row['Sector']} → {row['Currency']}<br>{v:,.0f} mln AMD ({pct:.1f}%)")
...
fig.update_layout(
    title=dict(text="The structure of residents' loans in Armenia (as of July 2025)", x=0.5, xanchor="center"),
...
out_path = "D:/MFS/Works/Articles/Loans/loans_alluvial.html"
